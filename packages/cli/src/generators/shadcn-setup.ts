...
async function runShadcnInit(
  ...
  if (config.shadcn.preset === "custom") {
    if (!config.shadcn.customPresetCode) {
      logger.warn(
        'Custom preset selected but no code provided; falling back to "nova".',
      );
    }
  ...
  try {
    ...
    loadingSpinner.fail(
      "shadcn init failed — skipping. You can run it manually later.",
    );
    return false;
  }
}
...
function deriveFallbackStyle(shadcnConfig: ProjectConfig["shadcn"]): string {
  // Custom preset codes are encoded configs, not preset names;
  // use a safe default instead
  const preset = shadcnConfig.preset === "custom" ? "nova" : shadcnConfig.preset;
 ...
 */
async function moveFilesToUiPackage(
  firstFrontendAppDir: string,
  uiDir: string,
) {
 ...
 */
async function writeAppGlobalsCss(
  targetDir: string,
 ...
 */
function buildUiComponentsJson(generatedConfig: {
  style: string;
  iconLibrary: string;
  baseColor: string;
}): object {
 ...
 *
 * Note: Next.js 16 already provides __dirname in config files, so we use
 * it directly without redeclaring.
 */
...
export async function generateShadcnSetup(
  ...
  if (shadcnSucceeded) {
    ...
    const generatedConfig = await readShadcnGeneratedConfig(
      componentsJsonPath,
      config.shadcn,
    );
